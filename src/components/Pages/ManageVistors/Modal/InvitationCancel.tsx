...
const InvitationCancelModal = ({
    ...
    const cancelInvitationApi = () => {
        axiosInstance
            .put(`/admin/visitors/cancel/${id}`)
            .then((response) => {
                console.log(response);
                dispatch(fetchVisitors(initialStates, 1));
                showToastMessage("Visitor Cancelled Successfully", "success");
                onCloseClick();
            })
            .catch((error) => {
                console.log(error);
                    ...
                    <div className="lg:p-5 md:p-5 p-4 flex flex-col lg:w-[671px] md:w-[671px] w-[500px] m-auto">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-sm font-nunitoMedium text-[#2B2C34] mt-2">
                                By canceling this invitation, the visitor will
                                be notified that the event has been canceled.
                            </p>

                            <div className="flex flex-row gap-5 justify-end items-end mt-5">
                                <button
                                    // onClick={onDeleteConfirm}
                                    onClick={cancelInvitationApi}
                                    className="w-[201px] h-[47px] bg-[#FF7B7B] rounded-lg hover:shadow-sm"
                                >
                                    <p className="text-sm font-bold text-darkbg font-nunitoRegular text-white">
                                        Cancel Invitation
                                    </p>
                                </button>
